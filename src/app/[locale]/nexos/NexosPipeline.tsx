...
import { PHASE_IDS } from "@/lib/nexos-data";

export function NexosPipeline() {
  const t = useTranslations("nexos.architecture");
          ...
          {PHASE_IDS.map((id, i) => (
            <FadeIn key={id}>
              <div className="flex items-center gap-4 rounded-sm border border-surface-border bg-surface p-5 transition-all duration-200 hover:border-text-tertiary/50">
                <span
                  aria-hidden="true"
                  className="flex h-8 w-8 shrink-0 items-center justify-center rounded-sm bg-accent/10 font-mono text-xs font-bold text-accent"
                >
